  ...
  server.get('/add-to-slack', bodyParser.json(), (req, res) => {
    ...
    slack.oauth.access(params, (err, oauthAccess) => {
      ...
      slack.auth.test({ token: oauthAccess.access_token }, (err, authTest) => {
        ...
        db.Team.upsert({teamId: oauthAccess.team_id, botUserid: oauthAccess.bot.bot_user_id, botToken: oauthAccess.bot.bot_access_token, appToken: oauthAccess.access_token})
